...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp,const double &mLatitude, const double &mLongitude)
{
    mImGray = im;
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
        mCurrentFrame = Frame(mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mLatitude,mLongitude);
    else
        mCurrentFrame = Frame(mImGray,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mLatitude,mLongitude);
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            // Local Mapping is activated. This is the normal behaviour                 
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    // In last frame "visual odometry" points are tracked.

                    // We compute two camera poses, one from motion model and one doing relocalization.
                    // If relocalization is not succeeded, "visual odometry" solution will be selected.
        ...
        else
        {
            ...
            // In this case, TrackLocalMap() is not possible to perform
            if(bOK && !mbVO)
                bOK = TrackLocalMap();
        }
        ...
        // If tracking were good, check if a keyframe is possible to insert.
        if(bOK)
        {
            // Update motion model
            ...
            // Check each Map point if it is outlier or not. 
            // If the Map point is outiler, than set the point as a NULL
            for(int i=0; i<mCurrentFrame.N;i++)
            {
                if(mCurrentFrame.mvpMapPoints[i] && mCurrentFrame.mvbOutlier[i])
                    mCurrentFrame.mvpMapPoints[i]=static_cast<MapPoint*>(NULL);
...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        ...
        if(mpInitializer->Initialize(mCurrentFrame, mvIniMatches, Rcw, tcw, mvIniP3D, vbTriangulated))
        {
            ...
            // std::cout<<"before create init"<<std::endl;
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    // At first, an ORB matching with the reference keyframe is performed.
...
void Tracking::UpdateLastFrame()
{
    ...
    // Sorting points according to measured depth by the RGB-D sensor
    ...
    // If less than 100 close points, than insert the 100 closest ones.
    int nPoints = 0;
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    //True, True -> nTrackedClose<0, nNonTrackedClose>70
   ...
   // // Condition 1a: More than "MaxFrames" have passed from last keyframe insertion
    ...
    const bool c3 = (mnMatchesInliers>15);
    ...
    if(c3)
    {
        ...
        {
            // If Current frame doesn't move current distance, return false
            if(c3)
                return true;
            else
                return false;
...
{
    std::cout<<"New Keyframe Upload"<<endl;
